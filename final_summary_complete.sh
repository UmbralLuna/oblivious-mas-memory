...
print("---------|-----------|---------|----------")
for r in results:
    print(f"   {r['parties']}     | {r['total_time_minutes']:.2f}分钟  | {r['transfer_size_gb']:.2f}GB  | {r['verify_time_seconds']}秒")
PY
...
with open('experiments/sanitization_validation/test_results.json') as f:
    data = json.load(f)
print(f"✓ 测试样本: 100个")
print(f"✓ 不合格脱敏检测率: {data['detection_rate']:.1f}% ({data['detected_count']}/{data['unqualified_count']})")
print(f"✓ 边界情况漏报率: {data['miss_rate']:.1f}% ({data['missed_count']}/{data['boundary_count']})")
