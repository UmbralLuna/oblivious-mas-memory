// circuits/ablation/delegate_no_g5.circom
// 消融变体：移除 G5 全部（标签一致性约束）
// 规范 v4.0 §4.4
// 预期退化：tag relaxation 攻击成功率 0% → > 30%
...
template DelegationNoG5(N_a, d_s) {
    signal input h_c;
    signal input pk_issuer[2];
    signal input nullifier;
    signal input current_time;

    signal input parent_iss, parent_hld, parent_scope_root;
    signal input parent_perm, parent_depth, parent_exp;
    signal input parent_attr_hash, parent_tc_hash;
    signal input parent_tc[3];

    signal input child_iss, child_hld, child_scope_root;
    signal input child_perm, child_depth, child_exp;
    signal input child_attr_hash, child_tc_hash;
    signal input child_tc[3];
    signal input randomness;

    signal input sig_R8x, sig_R8y, sig_S;
    signal input holder_sk, holder_pk_x, holder_pk_y;
...
component main { public [h_c, pk_issuer, nullifier, current_time] } = DelegationNoG5(16, 6);